        ...
        <div className="grid gap-4">
            {activeScans.map((scan) => {
                ...
                const duration = scan.endTime && scan.startTime
                    ? Math.round((scan.endTime - scan.startTime) / 1000)
                    : null;
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <span className="font-mono text-foreground truncate block" title={scan.config?.customArgs}>
                                            {scan.config?.templateId
                                                ? scan.config.templateId
                                                : scan.config?.tags
                                                    ? `Tags: ${Array.isArray(scan.config.tags) ? scan.config.tags.join(', ') : scan.config.tags}`
                                                    : scan.config?.severity
                                                        ? `Severity: ${Array.isArray(scan.config.severity) ? scan.config.severity.join(', ') : scan.config.severity}`
